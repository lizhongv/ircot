...
def non_cached_llm_call(
    ...
    # kwargs doesn't work with caching.
    params = {
        "prompt": prompt,
        "max_input": max_input,
...
class LLMClientGenerator:
    ...
    def generate_text_sequence(self, prompt):
        ...
        logger.debug(f"Calling LLM with params:\n{LYELLOW}{json.dumps(params, indent=4)}{RESET}")
        result = llm_call(**params)  # 调用大模型生成回复
        ...
        for text in generated_texts:
            ...
            if self.eos_text and self.eos_text in text:
                text = text[: text.index(self.eos_text)]  # TODO 按'\n'分割未必正确
            modified_texts.append(text)
        generated_texts = modified_texts

        output_seq_score = [(text, 1 / (index + 1)) for index, text in enumerate(generated_texts)]  # TODO 评分方式待修改
