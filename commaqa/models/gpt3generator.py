...
def openai_call(
    ...
    response = client.chat.completions.create(
        model="qwen-turbo",  # "deepseek-v3"  "qwen-plus"
        messages=[
        ...
        stop=stop,
        n=n,
        # best_of=best_of, TODO
        logprobs=logprobs,
    )
    # print(response.model_dump_json())
    # print(response.choices[0].message.content)
    # print(response.usage)
    # result = json.load(response.model_dump_json())
    return response
...
def get_gpt_tokenizer():
    ...
    return GPT2Tokenizer.from_pretrained("./deepseek_v3_tokenizer")
...
class GPT3Generator:
    def __init__(
        ...
        if "deepseek-v3" not in engine:
            raise Exception("Not allowed to prevent accidental $$ wastage.")

        if "deepseek-v3" not in engine and self.retry_after_n_seconds is not None:
            raise Exception("Retry is only supported for code-davinci as it's free. Using it for other paid models is risky and so is disabled.")

        if "deepseek-v3" in engine:
    ...
    def generate_text_sequence(self, prompt):
        ...
        prompt = fit_prompt_into_given_limit(
            ...
            tokenizer_model_name="deepseek-v3",  # TODO did this before tiktoken was released.
