...
def parse_arguments():
    ...
    arg_parser.add_argument("--reader", type=str, required=False, help="Dataset reader", choices=READER_NAME_CLASS.keys())
    ...
    arg_parser.add_argument("--variable-replacements", type=str, help="json string for jsonnet local variable replacements.", default="",)
...
def build_decomposer_and_models(config_map):
    ...
    for key, value in config_map["models"].items():
        ...
        if class_name not in MODEL_NAME_CLASS:
            raise ValueError("No class mapped to model name: {} in MODEL_NAME_CLASS: {}".format(class_name, MODEL_NAME_CLASS))

        logger.debug(f"Instantiating model of type: {LYELLOW}{class_name}{RESET} with config: {LYELLOW}{json.dumps(value, indent=4)}{RESET}")
        model = MODEL_NAME_CLASS[class_name](**value)  # TODO

        if key in config_map:
            raise ValueError("Overriding key: {} with value: {} using instantiated model of type: {}".format(key, config_map[key], class_name))
...
def demo_mode(args, reader, decomposer):
    ...
    while True:
        ...
        final_state, other_states = decomposer.find_answer_decomp(
            example, debug=args.debug)
...
def inference_mode(args, reader, decomposer, model_map, override_answer_by=None):
    ...
    if args.threads > 1:
        ...
        for example in tqdm(iterator, desc="Processing samples..."):
            ...
            qid_answer_chains.append(tmp)

    end_time = time.time()
    seconds_taken = round(end_time - start_time)

    predictions = {x[0]: x[1] for x in qid_answer_chains}
    logger.info(f"Writing predictions in {LYELLOW}{args.output}{RESET}")
    ...
    logger.debug(f"Writing time taken in {LYELLOW}{time_taken_file_path}{RESET}")
    ...
    logger.debug(f"Writing chains in {LYELLOW}{chain_tsv}{RESET}")
    ...
    logger.debug(f"Writing full_eval_path in {LYELLOW}{full_eval_path}{RESET}")
    ...
    logger.debug(f"Writing variable_replacements in {LYELLOW}{variable_replacements_path}{RESET}")
...
if __name__ == "__main__":
    ...
    if parsed_args.openai_api_keyname is not None:
        assert os.environ.get(parsed_args.openai_api_keyname, ""), f"No API key found in the env variable named: {parsed_args.openai_api_keyname}."
        ...
        logger.debug(f"OpenAI API key name used: {parsed_args.openai_api_keyname}")

    if parsed_args.llm_server_key_suffix:
        os.environ["LLM_SERVER_KEY_SUFFIX"] = parsed_args.llm_server_key_suffix

    logger.debug(f"Loading config from {LBLUE}{parsed_args.config}{RESET}")
    config_map = load_config(parsed_args.config)

    logger.debug(f"Building decomposer and models from config_map: {LYELLOW}{json.dumps(config_map['models'], indent=4)}{RESET}")
    decomposer, model_map = build_decomposer_and_models(config_map)

    logger.debug(f"Loading reader: {LYELLOW}{parsed_args.reader}{RESET}")
