...
def get_tokenizer(model_name):
    ...
    if model_name == 'google/flan-t5-large':  # TODO
        model_name = '/data1/Public/LLMs/flan-t5-large'
    elif model_name == 'google/flan-t5-xxl':  # TODO
        model_name = '/data1/Public/LLMs/flan-t5-xxl'
    elif model_name == 'deepseek-v3':  # TODO
        model_name = './deepseek_v3_tokenizer'
    elif model_name == 'Qwen/Qwen2.5-7B':  # TODO 
        model_name = '/data1/Public/LLMs/Qwen2.5-7B-Instruct'
    else:
...
def read_prompt(
    ...
    tokenizer_model_name: str = "deepseek-v3",  # TODO
    ...
    for index, line in tqdm(enumerate(all_prompt_lines), desc="Parsing Prompt", total=len(all_prompt_lines)):
        ...
        if line.strip().startswith(metadata_prefix):
            ...
            example = copy.deepcopy(metadata)  # copy
            example["lines"] = []
        else:
        ...
        for key, valid_values in filter_by_key_values.items():
            ...
            for example in examples:
                ...
                if key not in example:
                    logger.info(f"WARNING: Key {key} not found in the prompt file_path ({file_path}). Skipping it.")
                    continue
    ...
    if len(prompt_examples_texts) == 1:
        ...
        while prompt_example_lengths:
            ...
            estimated_total_length = (sum(prompt_example_lengths) + estimated_test_example_length + estimated_generation_length)
        ...
        if not prompt_examples_texts:
            logger.info("EXTREME WARNING: Not prompt examples remain.")

        if prompt_examples_dropped > 0:
            logger.info(f"WARNING: Dropped {prompt_examples_dropped} / {prompt_examples_original} examples.")
