...
class AnswerExtractor(ParticipantModel):
    ...
    def query(self, state, debug=False):
        logger.info("Run answer extractor")
        self.num_calls += 1
...
class RetrieveAndResetParagraphsParticipant(ParticipantModel):
    def __init__(
        ...
        if valid_titles_are_allowed_titles:
            assert retrieval_type == "bm25", "valid_titles_are_allowed_titles is applicable only when retrieval_type is bm25."

        if set_result_as_valid_titles:
            assert retrieval_type == "map_generated_to_valid_titles", "set_result_as_valid_titles is only available for map_generated_to_valid_titles retrieval type."
    ...
    def query(self, state, debug=False):
        logger.debug("Run retrieve and reset paragraphs")
...
class CopyQuestionParticipant(ParticipantModel):
    ...
    def query(self, state, debug=False):
        logger.debug("Run copy question")
...
class StepByStepLLMTitleGenParticipant(ParticipantModel):
    ...
    def query(self, state, debug=False):
        logger.debug("Run step by step retrieve")
