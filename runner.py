...
from log.logging_config import logger, LYELLOW, LRED, LBLUE, RESET, LGREEN
...
def main():
    ...
    parser.add_argument("model", type=str, choices=("qwen2.5-7b", "codex", "flan-t5-xxl", "flan-t5-xl", "flan-t5-large", "flan-t5-base", "none"))
    ...
    parser.add_argument("command", type=str, help="command",
                        choices={"print", "write", "verify", "predict", "evaluate", "track", "summarize", "ground_truth_check", "backup", "print_backup", "recover_backup", "delete_predictions", },)
    # 可选参数
    parser.add_argument("--prompt_set", type=str, help="prompt_set", choices={"1", "2", "3", "aggregate"}, default="1",)
    ...
    parser.add_argument("--only_print", action="store_true", default=False, help="print only for eval, ignore otherwise.")
    parser.add_argument("--force", action="store_true", default=False, help="force predict if it exists")
    parser.add_argument("--official", action="store_true", default=False, help="use official evaluation for evaluate and summarize.")
    ...
    # Build command
    run_command_array = [f"python run.py {args.command} {experiment_name} --instantiation_scheme {instantiation_scheme} --prompt_set {args.prompt_set}",]
    ...
    if (args.command in ("predict", "evaluate", "track", "summarize", "ground_truth_check") and not args.skip_evaluation_path) or args.best:
    ...
    if (args.command in ("predict", "summarize") or (args.command == "write" and args.best)) and train_dataset != eval_dataset:
    ...
    if not args.dry_run:
        ...
        logger.info(f"Run Command: {LRED}{run_command_str}{RESET}")
