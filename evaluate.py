...
def load_ground_truths(
    ...
    # prep ground_truths
    id_to_ground_truths = {}
    iterator = reader.read_examples(ground_truth_file_path)
...
def main():
    ...
    parser.add_argument("--question-type-key-value", type=str, help="':' separated question-type-key-value.", default=None)
    parser.add_argument("--only-print", action="store_true", default=False, help="only print don't run evaluation")
    parser.add_argument("--official", action="store_true", default=False, help="use official eval scripts when available.")
    ...
    # TODO
    prediction_directory = os.path.join("new_predictions", experiment_name + args.prediction_suffix)
    ...
    save_metrics_path = os.path.join(prediction_directory, official_prefix + "evaluation_metrics__" + prediction_file_name + ".json")
    if args.only_print:
        if not os.path.exists(save_metrics_path):
            exit("Asked to print the metrics, but the metrics file_path is not available.")
        logger.info(f"Loading the evaluation results from {LYELLOW}{save_metrics_path}{RESET}")
        with open(save_metrics_path, "r") as file:
            evaluate_results = json.load(file)
        logger.info(f"Evaluation results: {LYELLOW}{json.dumps(evaluate_results, indent=4)}{RESET}")
        exit("Done.")

    logger.info(f"Loading the experiment config from {LYELLOW}{config_filepath}{RESET}")
    ...
    logger.info(f"Loading the ground_truths from {LYELLOW}{args.evaluation_path}{RESET}")
    id_to_ground_truths = load_ground_truths(experiment_config, args.evaluation_path, question_type_key=question_type_key, question_type_value=question_type_value,)

    logger.info(f"Loading the predictions from {LYELLOW}{prediction_file_path}{RESET}")
    id_to_predictions = load_predictions(prediction_file_path)

    if question_type_value is not None:
        id_to_predictions = {id_: prediction for id_, prediction in id_to_predictions.items() if id_ in id_to_ground_truths.keys()}

    assert set(id_to_ground_truths.keys()) == set(id_to_predictions.keys()), "Ids in ground truths and predictions don't match"
    ...
    logger.info(f"Load the git-hash from {LYELLOW}{git_hash_filepath}{RESET}")
    ...
    logger.info(f"Evaluation results: {LYELLOW}{json.dumps(evaluation_results, indent=4)}{RESET}")

    logger.info(f"Saving metrics in {LYELLOW}{save_metrics_path}{RESET}")
    with open(save_metrics_path, "w") as file:
        json.dump(evaluation_results, file, indent=4)

    ground_truth_in_dict_file_path = os.path.join(prediction_directory, "ground_truth__" + prediction_file_name + ".json")
    logger.info(f"Save the ground_truth in {LYELLOW}{ground_truth_in_dict_file_path}{RESET}")
