...
def verify_config(config_file_path: str) -> bool:
    ...
    config_json = json.loads(_jsonnet.evaluate_file(
        config_file_path, ext_vars=env_variables))
    ...
    missing_paths = [
        validate_path for validate_path in validate_paths if not os.path.exists(validate_path)]
    if missing_paths:
...
def instatiate_config(content: str, variable_replacements: Dict[str, str],) -> str:
    # NOTE: It's updated in-place with evaluated replacements when required.
    """根据提供的变量替换字典，对输入的配置文件内容进行变量替换."""

    for variable_name, variable_value in variable_replacements.items():
        assert isinstance(variable_value, str)

        # 对 variable_value 中以$开头的变量占位符进行替换
        for invoked_variable_name in re.findall(r"\$[a-zA-Z0-9-_]+", variable_value):
            # 占位符，占位符是否存在于替换名单中
            invoked_variable_name = invoked_variable_name.lstrip("$")
            assert (invoked_variable_name in variable_replacements), "Invoked a variable name replacement within another that's not available in the passed dict."
        ...
        if original_variable_value.strip() == "null":
            raise Exception(f"Looks like you're trying to replace variable ({variable_name}) that is set to be none. Likely an error.")

        # Updated inplace.
        variable_replacements[variable_name] = variable_value
        content = re.sub(regex, r"\1 " + variable_value + r"\3", content)

    return content


def parse_dataset(content: str) -> str:
...
def is_experiment_complete(
    ...
    used_experiment_file_path = os.path.join(
        os.path.dirname(prediction_file_path),
        "config__" + os.path.splitext(os.path.split(prediction_file_path)[1])[0].replace("prediction__", "") + ".jsonnet",
    )
...
def main():
    ...
    # 可选参数
    base_parser.add_argument("--instantiation_scheme", type=str, help="instantiation_scheme", choices=instantiation_schemes.keys(), required=True,)
    base_parser.add_argument("--prompt_set", type=str, help="prompt_set", choices={"1", "2", "3", "aggregate"}, required=True)

    # 子命令write解析器
    subparsers = parser.add_subparsers(title="Commands", metavar="", dest="command")
    write_subparser = subparsers.add_parser("write", description="write files.", help="write files.", parents=[base_parser])
    ...
    write_subparser.add_argument("--variable_replacements", type=str, help="json string for jsonnet local variable replacements.", default="", )

    subparsers.add_parser("diff", description="show diff between original and new files.", help="show diff between original and new files.", parents=[base_parser],)
    subparsers.add_parser("verify", description="verify all required prompt files exist.", help="verify all required prompt files exist.", parents=[base_parser],)

    # 子命令print解析器
    print_subparser = subparsers.add_parser("print", description="print file names.", help="print file names.", parents=[base_parser])
    ...
    # 子命令backup解析器
    backup_subparser = subparsers.add_parser("backup", description="backup output directory.", help="backup output directory.", parents=[base_parser])
    backup_subparser.add_argument("--force", action="store_true", help="remove backup first if it exists.")

    # 子命令print_backup解析器
    print_backup_subparser = subparsers.add_parser("print_backup", description="print backup output directory if it exists.",
                                                   help="print backup output directory if it exists.", parents=[base_parser],)
    print_backup_subparser.add_argument("--force", action="store_true", help="remove original first if it exists.")

    # 子命令recover_backup解析器
    recover_backup_subparser = subparsers.add_parser("recover_backup", description="recover backup output directory to original directory.",
                                                     help="recover backup output directory to original directory.", parents=[base_parser],)
    recover_backup_subparser.add_argument("--force", action="store_true", help="remove original first if it exists.")

    # 子命令predict解析器
    predict_subparser = subparsers.add_parser("predict", description="run prediction on eval files.", help="run prediction on eval files.", parents=[base_parser],)
    predict_subparser.add_argument("--variable_replacements", type=str, help="json string for jsonnet local variable replacements.", default="",)
    ...
    subparsers.add_parser("delete_predictions", description="delete predictions directory.", help="delete predictions directory.", parents=[base_parser],)

    track_subparser = subparsers.add_parser("track", description="track progress of completion.", help="track progress of completion.", parents=[base_parser],)
    ...
    evaluate_subparser = subparsers.add_parser("evaluate", description="evaluate prediction on eval files.", help="evaluate prediction on eval files.", parents=[base_parser],)
    ...
    evaluate_subparser.add_argument("--question-type-key-value", type=str, help="':' separated question-type-key-value.", default=None)
    ...
    evaluate_subparser.add_argument("--only_print", action="store_true", default=False, help="only print don't run evaluation")
    evaluate_subparser.add_argument("--official", action="store_true", default=False, help="use official eval scripts when available.")

    summarize_subparser = subparsers.add_parser("summarize", description="summarize results of evaluation runs.", help="summarize results of evaluation runs.", parents=[base_parser],)
    ...
    summarize_subparser.add_argument("--variable_replacements", type=str, help="json string for jsonnet local variable replacements.", default="",)
    summarize_subparser.add_argument("--official", action="store_true", default=False, help="use official eval scripts when available.")

    ground_truth_check_subparser = subparsers.add_parser("ground_truth_check", description="prints head of all ground-turths to check equality.",
                                                         help="prints head of all ground-turths to check equality.", parents=[base_parser],)
    ...
    # I always select the best hp-config based on prompt_set 1, but replace the prompt examples to use based on what prompt_set is passed.
    # For prompt_set_1 I'll have __best suffix. For the rest, I'll have __best_p1_to_p2, and __best_p1_to_p3 suffixes.
    # I know the naming is a bit odd, but due to legacy reasons.
    ...
    # prompt_set 1,2,3 or aggregate
    _prompt_set = "1" if args.prompt_set == "aggregate" else args.prompt_set

    dataset_name = parse_dataset(file_content)
    ...
    # cot prompt
    _names = [f"prompt_set_{_prompt_set}"] if valid_qids is not None else [""]
    ...
    cartesian_product_hyperparameter_combinations = [e for e in itertools.product(*list(instantiation_scheme.values()))]
    for values in tqdm(cartesian_product_hyperparameter_combinations, desc="Prcoessing HP variations...", total=len(cartesian_product_hyperparameter_combinations)):
        ...
        # add local_file_content to local_file_contents
        local_file_contents.append(local_file_content)

        # build instantiate path
        overall_local_name = "___".join(local_names).lstrip("_")
        local_file_path = os.path.join(hyperparameter_variations_directory, "____".join([base_config_name, overall_local_name]) + ".jsonnet")
        ...
        prediction_directory = os.path.join("new_predictions", experiment_name)  # TODO
        evaluation_path = args.evaluation_path if hasattr(args, "evaluation_path") else None
        path_map = {"Experiment Name": experiment_name, "Prediction Path": prediction_directory,
                    "Evaluation Path": evaluation_path, "Instantiate Path": local_file_path}
        logger.debug(f"PATH: {LYELLOW}{json.dumps(path_map, indent=4)}{RESET}")
        ...
        if args.command in ("predict", "evaluate", "track", "summarize", "ground_truth_check") or args_best_is_passed:
            ...
            metrics_file_path = os.path.join(prediction_directory, "evaluation_metrics__" + evaluation_file_name + ".json")
            logger.debug(f"Prediction File Path: {LYELLOW}{prediction_file_path}{RESET}")
            logger.debug(f"Metrics File Path: {LYELLOW}{metrics_file_path}{RESET}")
            logger.debug(f"Ground Truth File Path: {LYELLOW}{ground_truth_file_path}{RESET}")
            logger.debug(f"Evaluation Path: {LYELLOW}{evaluation_path}{RESET}")
        ...
        if ((args.command == "diff") or (args.command == "write" and not args.no_diff)) and not args.best:
            logger.info(f"Comparing {LYELLOW}{config_filepath}{RESET} and {LYELLOW}{local_file_path}{RESET}\n")
            diff_command = f"colordiff {config_filepath} {local_file_path}"
            logger.debug(f"RUN Command: {LYELLOW}{diff_command}{RESET}")
            subprocess.call(f"{diff_command}", shell=True)  # sudo apt install colordiff
        # 2
        ...
        # 6
            ...
            if not args.skip_if_exists or not is_experiment_complete(local_file_path, prediction_file_path, metrics_file_path, args.variable_replacements):
                logger.info(f"Run predict command: {LRED}{run_command}{RESET}")
                subprocess.call(run_command, shell=True)
            else:
                logger.info(f"Skipping as the experiment is already complete.")
        # 7
        elif args.command == "track" and not args_best_is_passed:
            if is_experiment_complete(local_file_path, prediction_file_path, metrics_file_path, args.variable_replacements):
        ...
        # 8
            ...
            logger.info(f"Summarizing {LYELLOW}{overall_local_name}{RESET}")

            if os.path.exists(metrics_file_path):
                logger.info(f"READ metrics file from {LYELLOW}{metrics_file_path}{RESET}")
                ...
                if "para_recall" in metrics and "avg_predicted_paras" in metrics:
                    metric_value = "@".join([str(round(metrics["title_recall"] * 100, 1)), str(round(metrics["avg_predicted_titles"], 1))])
                    metric_value += " | "
                    metric_value += "@".join([str(round(metrics["para_recall"] * 100, 1)), str(round(metrics["avg_predicted_paras"], 1))])
                else:
                    metric_value = " | ".join(
                        [  # Note that "|" is used to identify the first metric if required.
                            # First item will be used for knowing best HP.
                ...
                hyperparameter_metrics_datum["complete"] = is_experiment_complete(local_file_path, prediction_file_path, metrics_file_path, args.variable_replacements)
    ...
    if args.command == "track" and not args_best_is_passed:
        ...
        completion_percentage = (len(complete_experiment_names) / len(all_experiment_names) if all_experiment_names else 0.0)
        print("\nComplete Experiment Names:\n--------------------------")
        print("\n".join(complete_experiment_names))
        print("\nInComplete Experiment Names:\n----------------------------")
        print("\n".join(incomplete_experiment_names))
        print("\nOverall Stats:\n--------------")
        print(f"{len(complete_experiment_names)} / {len(all_experiment_names)} " f"({completion_percentage}) completed.")
    ...
    if args_best_is_passed and str(args.prompt_set) in ("1", "2", "3"):
        ...
        source_best_experiment_name = "____".join([base_config_name, args.instantiation_scheme, source_target_prefix + "best"])

        if str(args.prompt_set) == "1":
            target_best_experiment_name = source_best_experiment_name
        else:
            target_best_experiment_name = "____".join([base_config_name, args.instantiation_scheme, source_target_prefix + f"best_p1_to_p{args.prompt_set}"])

        source_write_best_config_file_path = os.path.join(hyperparameter_variations_directory, source_best_experiment_name + ".jsonnet")
        target_write_best_config_file_path = os.path.join(hyperparameter_variations_directory, target_best_experiment_name + ".jsonnet")
    ...
    if args.command == "summarize" and args_best_is_passed and str(args.prompt_set) in ("1", "2", "3"):
        ...
        metrics_file_path = os.path.join("predictions", target_best_experiment_name, official_prefix + "evaluation_metrics__" + evaluation_file_name + ".json",)
        ...
        if not os.path.exists(metrics_file_path):
            ...
            if "para_recall" in metrics and "avg_predicted_paras" in metrics:
                metric_value = "@".join(
                    [str(round(metrics["title_recall"] * 100, 1)),
                     str(round(metrics["avg_predicted_titles"], 1))]
                )
                metric_value += " | "
                metric_value += "@".join(
                    [str(round(metrics["para_recall"] * 100, 1)),
                     str(round(metrics["avg_predicted_paras"], 1))]
                )
            else:
                metric_value = " | ".join(
                    [  # Note that "|" is used to identify the first metric if required.
                        # First item will be used for knowing best HP.
                        str(round(metrics["f1"] * 100, 1)),
                        str(round(metrics["precision"] * 100, 1)
                            ) if "precision" in metrics else "----",
                        str(round(metrics["recall"] * 100, 1)
                            ) if "recall" in metrics else "----",
                        str(metrics["count"]).rjust(5, " "),
                    ]
                )
        prediction_file_name = os.path.splitext(os.path.basename(evaluation_path))[0]
        ...
        prediction_file_path = os.path.join(prediction_directory, "prediction__" + prediction_file_name + ".json")
        if not is_experiment_complete(target_write_best_config_file_path, prediction_file_path, metrics_file_path, args.variable_replacements):
    ...
    if args.command == "summarize" and args_best_is_passed and str(args.prompt_set) == "aggregate":
        ...
        for _prompt_set in ("1", "2", "3"):
            ...
            if str(_prompt_set) == "1":
                target_best_experiment_name = "____".join([base_config_name, args.instantiation_scheme, source_target_prefix + "best"])
            else:
                target_best_experiment_name = "____".join([base_config_name, args.instantiation_scheme, source_target_prefix + f"best_p1_to_p{_prompt_set}"])
    ...
    if args_best_is_passed and args.command == "write" and str(args.prompt_set) in ("1", "2", "3"):
        ...
        if str(args.prompt_set) == "1":
            ...
            print(f"    from: {LYELLOW}{best_config_file_path}{RESET}")
            print(f"      to: {LYELLOW}{source_write_best_config_file_path}{RESET}")
    ...
    if args_best_is_passed and args.command in ("predict", "evaluate") and str(args.prompt_set) in ("1", "2", "3"):
        ...
        prediction_directory = os.path.join("new_predictions", target_best_experiment_name)  # TODO
        if hasattr(args, "use_backup") and args.use_backup:
            prediction_directory += "__backup"
        if evaluation_path is None:
            exit("Pass evaluation_path or set a default one in the hp_manager.")
        ...
        metrics_file_path = os.path.join(prediction_directory, official_prefix + "evaluation_metrics__" + evaluation_file_name + ".json")
        ...
        if args.command == "predict":
            ...
            if not args.skip_if_exists:
                logger.info(f"Run command: {LYELLOW}{run_command}{RESET}")
                subprocess.call(run_command, shell=True)
            else:
                if not is_experiment_complete(local_file_path, prediction_file_path, metrics_file_path, args.variable_replacements):
                    logger.info(f"Run command: {LYELLOW}{run_command}{RESET}")
                    subprocess.call(run_command, shell=True)
